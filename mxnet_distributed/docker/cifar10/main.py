...
parser.add_argument('--data-dir', type=str, default='',
                    help='training directory of cifar10 images, contains train/val subdirs.')
parser.add_argument('--batch-size', type=int, default=32,
                    help='training batch size per device (CPU/GPU).')
parser.add_argument('--num-worker', '-j', dest='num_workers', default=1, type=int,
...
def get_data_iter(data_dir, batch_size, num_workers=1, rank=0):
    data_shape = (3, 32, 32)
    resize = -1
    train = mx.io.ImageRecordIter(
        path_imgrec = Path(data_dir) / "train.rec",
    ...
    val = mx.io.ImageRecordIter(
        path_imgrec = Path(data_dir) / "test.rec",
        # mean_img    = "data/cifar/mean.bin",
        resize      = resize,
        rand_crop   = False,
        rand_mirror = False,
...
def train(opt, ctx):
    ...
    logger.info('Getting data iter')
    train_data, val_data = get_data_iter(opt.data_dir, batch_size, kv.num_workers, kv.rank)
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        for i, batch in enumerate(train_data):
            logger.info('Epoch[%d] Batch [%d]\tSplit and load batch'%(epoch, i))
...
def main():
    print("KVSTORE: " + opt.kvstore)
    logger.info('KVStore param: ' + opt.kvstore)
